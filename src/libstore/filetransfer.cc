...
/* Cause this thread to be notified on SIGINT. */
    ...
    ItemHandle enqueueItem(ref<TransferItem> item)
    {
        ...
        if (scheme != "http" && scheme != "https" && scheme != "ftp" && scheme != "ftps" && scheme != "file")
            throw nix::Error(
                "URL scheme '%s' is not supported; only http, https, ftp, ftps, and file are allowed (in '%s')",
                scheme,
                item->request.displayUri());
